/**
 * @description custom extension for indentation
 * @see {@link https://github.com/ueberdosis/tiptap/issues/1036}
 */
...
export const Indent = Extension.create<IndentOptions>({
  name: 'indent',
  defaultOptions: {
    types: ['paragraph', 'listItem', 'codeBlock'],
  ...
  addCommands() {
    return {
      ...
      outdent: (backspace = false) => ({ tr, state, dispatch }) => {
        const { selection } = state;
  ...
  addKeyboardShortcuts() {
    // assertion for ignore type checking
    return {
      Tab: ({editor}) => editor.commands.indent() as any,
      'Shift-Tab': ({editor}) => editor.commands.outdent(false) as any,
      Backspace: ({editor}) => editor.commands.outdent(true) as any,
    };
  },
});
